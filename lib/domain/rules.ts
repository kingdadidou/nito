...
export function notificationHref(type:string,data:unknown){const values=typeof data==="object"&&data?data as Record<string,unknown>:{};const tripId=typeof values.trip_id==="string"?values.trip_id:null;const senderId=typeof values.sender_id==="string"?values.sender_id:null;if(type==="nouveau_message_groupe"&&tripId)return `/messages?group=${tripId}`;if(type==="nouveau_message"&&senderId)return `/messages?with=${senderId}${tripId?`&trip=${tripId}`:""}`;if(["booking_confirmed","booking_cancelled"].includes(type))return "/reservations";if(["organizer_new_booking","participant_cancelled"].includes(type)&&tripId)return `/organisateur/sorties/${tripId}`;if(tripId)return `/sorties/${tripId}`;return "/notifications"}
