...
export default async function Reviews({searchParams}:{searchParams:Promise<{succes?:string;erreur?:string}>}){
  ...
  const {data:bookings}=await supabase.from("bookings").select("trip_id,booking_status,trip:trips(id,title,date,start_time,organizer_id,organizer:profiles!trips_organizer_id_fkey(first_name,last_name))").eq("participant_id",user.id).eq("booking_status","terminee");
  ...
  const eligible=(bookings??[]).flatMap(item=>{const trip=Array.isArray(item.trip)?item.trip[0]:item.trip;return !trip||writtenTrips.has(trip.id)?[]:[trip]});
