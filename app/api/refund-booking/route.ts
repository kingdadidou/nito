...
export async function POST(request:Request){
  try{
    const form=await request.formData();const bookingId=String(form.get("bookingId")??"");const reason=String(form.get("reason")??"Annulation");
    ...
    return NextResponse.redirect(new URL("/calendrier?remboursement=demande",request.url),303);
