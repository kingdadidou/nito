...
export async function POST(request:Request){
  try{
    ...
    if(booking.payment_status!=="paye"||!booking.stripe_payment_intent_id)return NextResponse.json({error:"Ce paiement ne peut pas être remboursé"},{status:409});
    const refund=await getStripe().refunds.create({payment_intent:booking.stripe_payment_intent_id,reverse_transfer:true,refund_application_fee:true,reason:"requested_by_customer",metadata:{booking_id:booking.id,reason}},{idempotencyKey:`refund:${booking.id}:full`});
    ...
    if(profile?.user_type==="administrateur")await admin.from("admin_audit_logs").insert({admin_id:user.id,action:"create_refund",entity_type:"booking",entity_id:booking.id,details:{refund_id:refund.id,amount:Number(booking.amount),reason}});
    const returnTo=profile?.user_type==="administrateur"&&requestedReturn==="/administration/reservations"?"/administration/reservations?remboursement=demande":"/calendrier?remboursement=demande";
