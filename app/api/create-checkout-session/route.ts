...
export async function POST(request:Request){
  ...
  }catch(error){console.error("Checkout Connect",error);const value=error&&typeof error==="object"?error:null;const rawCode=value&&"code" in value&&value.code?String(value.code):value&&"type" in value&&value.type?String(value.type):value&&"name" in value?String(value.name):"inconnue";const code=`${stage}:${rawCode}`;const target=/^[0-9a-f-]{36}$/i.test(tripId)?`/sorties/${tripId}?paiement=erreur&code=${encodeURIComponent(code)}`:"/explorer?paiement=erreur";return NextResponse.redirect(new URL(target,request.url),303);}
}
