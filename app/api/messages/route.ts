...
export async function POST(request:Request){
  ...
  const uploadToken=crypto.randomUUID();const admin=createAdminClient();let uploadedPath:string|null=null;
  if(file instanceof File&&file.size>0){const ext=file.type==="application/pdf"?"pdf":file.type==="image/jpeg"?"jpg":file.type==="image/png"?"png":"webp";uploadedPath=`${user.id}/${uploadToken}/${crypto.randomUUID()}.${ext}`;const {error}=await admin.storage.from("message-attachments").upload(uploadedPath,new Uint8Array(await file.arrayBuffer()),{contentType:file.type});if(error)return NextResponse.json({error:"La pièce jointe n’a pas pu être envoyée"},{status:500});}
