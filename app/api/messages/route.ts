...
const allowedAttachments=new Set(["application/pdf","image/jpeg","image/png","image/webp"]);
...
export async function POST(request:Request){
  ...
  if(!receiverId||!content||content.length>4000)return NextResponse.json({error:"Message invalide"},{status:400});
  ...
  if(file instanceof File&&file.size>0){
    if(!allowedAttachments.has(file.type)||file.size>10_485_760)return NextResponse.json({error:"Pièce jointe invalide"},{status:400});
