...
export default async function Messages({searchParams}:{searchParams:Promise<{with?:string;trip?:string}>}){
  ...
  return <section className="page-shell"><DashboardNav role={viewer?.user_type??"participant"}/><MessagingClient userId={user.id} initialMessages={messages} profiles={(profiles??[]) as ChatProfile[]} initialPeer={q.with} initialTrip={q.trip}/></section>;
}
