...
export default async function Explore({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  let query=supabase?.from("trips").select("id,title,location,date,price,approximate_latitude,approximate_longitude,activity:activities(name),organizer:profiles!trips_organizer_id_fkey(first_name,last_name,average_rating),images:trip_images(public_url,alt_text,position)").eq("status","publiee").gte("date",new Date().toISOString().slice(0,10)).order("date");
